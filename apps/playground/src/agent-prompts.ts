...
Import typography from '@calebduren/typograph' and register [typography, ${pluginOptions(settings)}] only for responses known to be English. This is English-only, with one house style and no language detection. Other-language, mixed-language, or verbatim responses should opt out. If the app has no per-response language signal (a user or tenant language setting, the answer language in the system prompt, or a structured output field), register [typography, { locale: 'en', punctuation: { quotes: false, apostrophes: true } }] instead: the typographic apostrophe is correct across Latin-script languages, and quotation marks stay as written. Interface language alone is not a signal. ${settings.spacing ? 'Enable conservative unit, initial, and abbreviation joins. Leave shortWords and lastWords off.' : 'Leave whitespace unchanged.'} Keep plugin configuration stable between renders and preserve the original Markdown source for the Remark transform.
