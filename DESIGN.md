...
- **One accent.** Lime (#d5f27c) on olive (#2d3822) for the primary button and the “New” tag; in dark mode the pair inverts. Changed glyphs are marked with a translucent lime highlight; a no-break space adds a short olive bar beneath the gap.
- **Type.** Hedvig Letters Serif (roman, optical size 24) for headings and the wordmark only, at zero tracking so its ligatures (fi, fl, ff, ft, tt) render. Headings are one color; no accent words. Pairs with under 0.02em of clear space are kerned by hand with letter-spacing spans (`src/Kern.tsx`, generated by `scripts/measure-hedvig-kerning.py`); a span drops the font's kern on both sides, so each value restores it. Inter Variable, with its default glyphs (double-story a), for everything else, including every demonstrated reply. Monospace only for code and commands.
- **Mark and header.** A seven-stroke burst (`src/LogoMark.tsx`) beside the serif wordmark on the left; Changelog, GitHub, and Install on the right. The changelog lives at `/changelog`, typeset from `CHANGELOG.md` by the package.
...
- **The stream story.** The hero pins and recedes (scale to 1.16, blur to 18px, fade to 6%) as two plain columns scale up and rise over it: model output, and the same tokens under the typograph logo, in light (350) Inter with no cards, backgrounds, or caret. Streaming starts as the columns enter the viewport, so about 80% of it happens while they are still moving; they then pin for the last 20%. Each frame eases toward the token count the scroll sets, so a fast scroll streams fast, a pause stops mid-sentence, and scrolling back rewinds. The right column is `typesetText` in its streaming phase on each prefix, the last token uses the complete phase, and a paragraph that opens with a quote hangs it using the package's hanging classes. Both columns mark the same characters. Reduced motion removes pinning and zoom and shows the finished reply.
